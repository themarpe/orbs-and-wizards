...
void Orb::checkCollisionLevel(Level* level) {
    ...
    if(point.first >= 0 && point.first < level->getLevelWidth() / level->getTileSize() && point.second >= 0
       && point.second < level->getLevelHeight() / level->getTileSize()) {
            ...
            if(Game::checkBoxCollision(e->getCollisionRect(), &collisionBox)) {
                ...
                if(hittableEnemy != NULL && hittableEnemy->getHittable()) {
                    if(frezze && hittableEnemy->getFrezze()) {
                        ...
                        if(dir.x > 0) {
                            hittableEnemy->hit(power, Direction::RIGHT);
                        } else {
                            hittableEnemy->hit(power, Direction::LEFT);
                        }
                        if(!(orb == 2 || orb == 5) && orbStance != Stance::END) hit();
                    }
                }
...
void Orb::render(Camera* camera) {
    ...
    camera->RenderCopyEx(Game::SPRITESHEET.at(spriteSheetKey).getSheetTexture(),
                         Game::SPRITESHEET.at(spriteSheetKey).getSpriteRectAt(currentSprite),
                         &renderRect,
                         270 + angle,
                         0,
                         flip);
