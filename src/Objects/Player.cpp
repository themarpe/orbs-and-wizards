...
void Player::checkCollision(Level* level) {
    ...
    for(int c = 0; c < 8; c++) {
        ...
        if(j >= 0 && j < level->getLevelWidth() / 32 && i >= 0 && i < level->getLevelHeight() / 32 && level->levelData[i][j] != NULL
...
void Player::render(Camera* camera) {
    ...
    if(playerDirection == Direction::LEFT) {
        ...
        camera->RenderCopyEx(
            Game::SPRITESHEET.at("PlayerBlue").getSheetTexture(), Game::SPRITESHEET.at("PlayerBlue").getSpriteRectAt(currentSprite), &renderRect, 0, 0, flip);
    } else {
        camera->RenderCopy(
            Game::SPRITESHEET.at("PlayerBlue").getSheetTexture(), Game::SPRITESHEET.at("PlayerBlue").getSpriteRectAt(currentSprite), &renderRect);
    }
...
bool Player::castOrb(int orb) {
    ...
    if(orb <= 2) {
        if(playerDirection == Direction::LEFT) {
            castingOrb->setX(box->x - 12 - castingOrb->getW() / 2);
        } else {
            castingOrb->setX(box->x + box->w + 12 - (castingOrb->getW() / 2));
        }
    } else {
        if(playerDirection == Direction::LEFT) {
            castingOrb->setX(box->x - 24 - castingOrb->getW() / 2);
        } else {
            castingOrb->setX(box->x + box->w + 12 - (castingOrb->getW() / 4));
        }
    }
