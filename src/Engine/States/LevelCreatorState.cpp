...
void LevelCreatorState::init(GameEngine* gameEngine) {
    ...
    levelCreator = new LevelCreator(renderer,
                                    gameEngine->levelCreatorData.levelWidth,
                                    gameEngine->levelCreatorData.levelHeight,
                                    gameEngine->levelCreatorData.tileSize,
                                    gameEngine->levelCreatorData.backgroundPath,
                                    gameEngine->levelCreatorData.tilesetPath);
...
void LevelCreatorState::update(GameEngine* gameEngine) {
    ...
    if(activeWindow1) {
        ...
        if(mouseButtonLeft) {
            ...
            if(mouseX > nameInputRect.x && mouseX < nameInputRect.x + nameInputRect.w && mouseY > nameInputRect.y
               && mouseY < nameInputRect.y + nameInputRect.h) {
            ...
            if(mouseY < tilesetRect.h) {
                ...
                int ny = mouseY;
                pointerGrabbedInt =
                    ny / levelCreator->getTileSize() * levelCreator->getTilesetWidth() / levelCreator->getTileSize() + nx / levelCreator->getTileSize();
                ...
                for(int i = 0; i < 10; i++) {
                    if(mouseX > iconsRect[i].x && mouseX < iconsRect[i].x + iconsRect[i].w && mouseY > iconsRect[i].y
                       && mouseY < iconsRect[i].y + iconsRect[i].h) {
...
void LevelCreatorState::render(GameEngine* gameEngine) {
    ...
    if(pointerGrabbedInt >= 0)
        SDL_RenderCopy(renderer, levelCreator->tileset.getSheetTexture(), levelCreator->tileset.getSpriteRectAt(pointerGrabbedInt), &redRect);
